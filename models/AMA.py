...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.):
        ...
        super().__init__()
        inner_dim = dim_head *  heads#计算内部维度 = 头数 × 每头维度
        project_out = not (heads == 1 and dim_head == dim)  # 如果只有1个头且dim_head=dim，则不需要投影
                                                            # 否则需要将inner_dim投影回原始dim

        self.heads = heads
        # scale = 1/√(dim_head)，用于缩放注意力分数
        # 例如：dim_head=64，则scale = 1/√64 = 1/8 = 0.125
        self.scale = dim_head ** -0.5

        self.attend = nn.Softmax(dim = -1)#softmax函数，将注意力分数归一化到0-1之间
        ...
        self.to_out = nn.Sequential(
            ...
            nn.Dropout(dropout)
        ) if project_out else nn.Identity()#如果需要投影：线性层(inner_dim→dim) + Dropout
                                           #如果不需要：恒等变换（Identity）

    def forward(self, q, k, v):
        b, n, _, h = *q.shape, self.heads#b: batch size, n: 序列长度, h: 头数
        q = self.to_q(q)
        k = self.to_k(k)
        v = self.to_v(v)
...
class AMAEncoder(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, dropout = 0.):
        super().__init__()
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, AMALayer(dim, heads = heads, dim_head = dim_head, dropout = dropout))
            ]))
    ...
    def forward(self, x, save_hidden=False):
        if save_hidden == True:
            ...
            for attn, ff in self.layers:
                #attn(x, x, x): 自注意力（Q=K=V=x）     + x: 残差连接
                x = attn(x, x, x) + x#注意力计算 + 残差连接
                x = ff(x) + x#前馈网络 + 残差连接
                hidden_list.append(x)
            return hidden_list
        else:
